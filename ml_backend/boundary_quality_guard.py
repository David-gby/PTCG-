...
def assess_inner_quality(inner: Mapping[str, Any]) -> dict[str, Any]:
    ...
    reasons: list[str] = []
    edges: list[str] = []
    physical = inner.get("physical_inner_prior", {}) if isinstance(inner, Mapping) else {}
    physical = physical if isinstance(physical, Mapping) else {}
    physical_after = physical.get("after", {}) if isinstance(physical, Mapping) else {}
    physical_after = physical_after if isinstance(physical_after, Mapping) else {}
    ...
    physical_risk = str(physical_after.get("risk") or "normal")
    if bool(physical_after.get("review_recommended", False)):
        reasons.append("physical_inner_size_residual_suggests_outer_or_inner_failure")
        edges.extend(["left", "right", "top", "bottom"])
    elif physical_risk == "moderate" and not bool(physical.get("applied", False)):
        reasons.append("physical_inner_size_residual_not_visually_supported")
        residual = physical_after.get("residual_px", {})
        residual = residual if isinstance(residual, Mapping) else {}
        if abs(float(residual.get("width", 0.0) or 0.0)) > 12.0:
            edges.extend(["left", "right"])
        if abs(float(residual.get("height", 0.0) or 0.0)) > 12.0:
    ...
    return {
        ...
        "edges": unique_edges,
        "reasons": reasons,
        "physical_prior_risk": physical_risk,
        "physical_prior_applied": bool(physical.get("applied", False)),
        "policy": "inner_boundary_quality_guard_20260822_v3",
    }
