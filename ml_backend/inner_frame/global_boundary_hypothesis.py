...
@dataclass(frozen=True)
class EdgePositionEvidence:
    """Card-spanning geometric evidence at one proposed edge position."""
...
def score_inner_edge_positions(
    image: np.ndarray,
    box: Mapping[str, float],
    edge: str,
    ...
    *,
...
) -> list[EdgePositionEvidence]:
    """Score proposals using robust support across the complete printed side."""
    ...
    if edge in ("left", "right"):
        aggregate, matrix = _segmented_profile(
            feature_x,
            vertical=True,
            long_low=top + long_y_pad,
            long_high=bottom - long_y_pad,
        ...
        dimension = height
    low, high = _band(edge, width, height)
    low = int(np.clip(low, 0, min(dimension, aggregate.size) - 2))
    high = int(np.clip(high, low + 1, min(dimension, aggregate.size) - 1))
    band_z = _robust_z(aggregate[low : high + 1])

    output: list[EdgePositionEvidence] = []
    for position in positions:
        center = int(np.clip(round(float(position)), low, high))
        begin = max(low, center - max(0, int(local_radius)))
        end = min(high, center + max(0, int(local_radius)))
        local = aggregate[begin : end + 1]
        snapped = begin + int(np.argmax(local)) if local.size else center
        segment_values = matrix[:, snapped]
        output.append(
            EdgePositionEvidence(
                edge=edge,
                position=round(float(position), 4),
                snapped_position=float(snapped),
                aggregate_score=round(float(aggregate[snapped]), 5),
                robust_z=round(float(band_z[snapped - low]), 5),
                supporting_segments=int(
                    np.count_nonzero(segment_values >= float(support_threshold))
                ),
                total_segments=segments,
                segment_median=round(float(np.median(segment_values)), 5),
                segment_q30=round(float(np.quantile(segment_values, 0.30)), 5),
            )
        )
    return output


def guarded_top_specialist_decision(
    *,
    base_position: float,
    stable_position: float,
    specialist_position: float,
    specialist_confidence: float,
    specialist_tta_disagreement: float,
    stable_evidence: EdgePositionEvidence,
    specialist_evidence: EdgePositionEvidence,
    config: Mapping[str, Any],
) -> dict[str, Any]:
    """Route only bounded, independently supported top-edge corrections."""

    move = float(specialist_position) - float(stable_position)
    stable_inward = float(stable_position) - float(base_position)
    evidence_margin = (
        float(specialist_evidence.aggregate_score)
        - float(stable_evidence.aggregate_score)
    )
    checks = {
        "directional_move": float(config["min_move"]) <= move <= float(config["max_move"]),
        "stable_not_already_inward": stable_inward <= float(config["max_stable_inward"]),
        "specialist_confident": float(specialist_confidence) >= float(config["min_confidence"]),
        "tta_stable": float(specialist_tta_disagreement) <= float(config["max_tta"]),
        "long_side_support": int(specialist_evidence.supporting_segments) >= int(config["min_support"]),
        "evidence_margin": evidence_margin >= float(config["min_evidence_margin"]),
        "robust_line_score": float(specialist_evidence.robust_z) >= float(config["min_robust_z"]),
    }
    accepted = all(checks.values())
    blend = float(config["blend"]) if accepted else 0.0
    return {
        "accepted": accepted,
        "position": round(float(stable_position) + blend * move, 4),
        "move_px": round(move, 4),
        "stable_inward_from_base_px": round(stable_inward, 4),
        "evidence_margin": round(evidence_margin, 5),
        "checks": checks,
        "reason": "guarded_top_consensus" if accepted else "guard_rejected",
    }


def guarded_top_tail_fallback_decision(
    *,
    base_position: float,
    selected_position: float,
    selected_confidence: float,
    selected_evidence: EdgePositionEvidence,
    config: Mapping[str, Any],
) -> dict[str, Any]:
    """Fall back to the independent base coordinate for top-edge tail risks."""

    move = float(selected_position) - float(base_position)
    large_weak_inward = bool(
        move >= float(config["inward_min_move"])
        and float(selected_evidence.robust_z) < float(config["inward_min_robust_z"])
    )
    large_uncertain_outward = bool(
        move <= -float(config["outward_min_move"])
        and float(selected_confidence) < float(config["outward_min_confidence"])
    )
    fallback = large_weak_inward or large_uncertain_outward
    if large_weak_inward:
        reason = "large_inward_move_without_robust_line_evidence"
    elif large_uncertain_outward:
        reason = "large_outward_move_with_low_model_confidence"
    else:
        reason = "tail_risk_not_detected"
    return {
        "fallback": fallback,
        "position": round(float(base_position) if fallback else float(selected_position), 4),
        "move_px": round(move, 4),
        "reason": reason,
        "checks": {
            "large_weak_inward": large_weak_inward,
            "large_uncertain_outward": large_uncertain_outward,
        },
        "selected_confidence": round(float(selected_confidence), 5),
        "selected_robust_z": round(float(selected_evidence.robust_z), 5),
        "selected_supporting_segments": int(selected_evidence.supporting_segments),
    }
