(function () {
  ...
  async function review(action) {
    ...
    if (action === "approve") {
      ...
      const predictedOuter = state.selected.prediction?.outer_corners;
      const predictedInner = state.selected.prediction?.inner_line_centers_px;
      const missingOuter = !(
        Array.isArray(predictedOuter)
        && predictedOuter.length === 4
        && predictedOuter.every(
          (point) => Array.isArray(point)
            && point.length === 2
            && point.every((value) => Number.isFinite(Number(value)))
        )
      );
      const missingInner = !predictedInner || !["left", "right", "top", "bottom"].every(
        (key) => Number.isFinite(Number(predictedInner[key]))
      );
      ...
      payload = {
        ...payload,
        corrected_inner: correctedInner,
        corrected_outer: correctedOuter,
        manual_completion_confirmed: missingOuter || missingInner,
      };
    }
    state.reviewInFlight = true;
    setReviewControlsDisabled(true);
