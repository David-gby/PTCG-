...
if [[ -z "${SOURCE_DIR}" || ! -f "${SOURCE_DIR}/VERSION" ]]; then
  echo "用法: sudo bash apply_v0.8.4_update.sh <解压后的 cardscope 目录>" >&2
  exit 2
fi
...
if [[ -z "${CURRENT_DIR}" || ! -d "${CURRENT_DIR}" ]]; then
  echo "没有找到 ${CURRENT_LINK}，请先确认服务器现有部署。" >&2
  exit 2
fi
if [[ ! -x "${VENV_PYTHON}" ]]; then
  echo "没有找到服务器 Python：${VENV_PYTHON}" >&2
  exit 2
fi

STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP_DIR="/opt/cardscope/backups/pre_v0.8.4_${STAMP}"
...
# Production state remains outside the release and must already be writable.
sudo -u cardscope test -w "${WORKSPACE_ROOT}/private"
systemctl restart "${SERVICE_NAME}"

for _ in $(seq 1 60); do
  if curl -fsS "${HEALTH_URL}" >/dev/null; then
    echo "CardScope v0.8.4 更新完成。"
